...
import { Car, Train, Bike, Footprints, ChevronDown } from 'lucide-react';
...
export default function MapView({ items, travelMode, onTravelModeChange, selectedItemId, isModalOpen }: MapProps) {
  const getMapUrl = () => {
    ...
    // If the first item is selected, just show that single place
    if (selectedIndex === 0) {
      const place = encodeURIComponent(items[0].place_name);
      return `https://maps.google.com/maps?q=${place}${baseParams}`;
    } 
    // If any other item is selected, show route from previous to selected
    ...
    // Fallback: if nothing is selected or only 1 item exists, show original behavior
    if (items.length === 1) {
      const place = encodeURIComponent(items[0].place_name);
      return `https://maps.google.com/maps?q=${place}${baseParams}`;
    }
